...
- `app/(tabs)/todo/index.tsx` — **built**: task list with scrollable filter bar (All / This Week / Overdue / Starred / **Completed** — Completed tab replaces the old show-completed toggle), pull-to-refresh, bidirectional done toggle (checkbox + undo toast), sort by due date or priority (header icon), mark-all-overdue-done bulk action, swipe-to-delete (hard delete for manual; soft delete for ICS/ai/syllabus), prominent **"Add task" pill button** in header (same style as Schedule's "Add block"), add-task form with title + **inline calendar date picker** (react-native-calendars `Calendar` expands inside the modal — no nested Modal, no text input) + priority pills; ICS import removed from this tab (lives in Profile); `__DEV__`-only sign-out icon; scroll resets to top on filter change; `isThisWeek` is today→+7 days only
